...
<head>
  <meta charset="UTF-8">
  <title>Admin Dashboard - IPL Players</title>
...
<!-- Section Title -->
<div class="section-title">Added Players</div>
...
%>
  <div class="alert alert-success text-center">✅ Player **added** successfully!</div>
<% } else if ("updated".equals(status)) { %>
  <div class="alert alert-success text-center">🔄 Player **updated** successfully!</div>
<% } else if ("deleted".equals(status)) { %>
  <div class="alert alert-info text-center">🗑️ Player deleted successfully!</div>
<% } else if ("updateError".equals(status)) { %>
  <div class="alert alert-danger text-center">❌ Failed to update player.</div>
<% } else if ("addError".equals(status)) { %>
  <div class="alert alert-danger text-center">❌ Failed to add player.</div>
<% } %>
<!-- Search Bar -->
...
<div class="container mt-3" id="playersContainer">
  ...
  <%= p.getPlayerName().toUpperCase() %> (#<%= p.getJerseyNumber() %>)
        ...
        <div class="action-icons">
          <a href="updatePlayer.jsp?jerseyNumber=<%= p.getJerseyNumber() %>&teamCode=<%= t.getTeamCode() %>"><i class="fas fa-edit"></i></a>
          <i class="fas fa-trash" onclick="confirmDelete(<%= p.getJerseyNumber() %>, '<%= t.getTeamCode() %>')"></i>
        </div>
      </div>
    </div>
    <%
...
<div class="floating-button">
  <a href="addPlayer.jsp"><i class="fas fa-plus-circle"></i> Add Player</a>
</div>
  <footer>
...
<script>
  function confirmDelete(jersey, team) {
    if (confirm("Are you sure you want to delete this player?")) {
      window.location.href = "PlayerServlet?action=delete&jerseyNumber=" + jersey + "&teamCode=" + team;
    }
  }
